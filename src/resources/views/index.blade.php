...
<div class="contact-form">
    ...
    <form action="{{ route('contact.confirm') }}" method="POST">
        ...
        <input type="email" name="email" value="{{ old('email') }}" placeholder="例: sample@example.com">
        @error('email')
            <div class="error">{{ $message }}</div>
        @enderror
        ...
        <div class="tel-group">
            <input type="text" name="tel1" maxlength="4" placeholder="090">
        ...
        <input type="text" name="building" value="{{ old('building') }}" placeholder="例: サンライズマンション203">
        ...
        <textarea name="content" rows="5" placeholder="お問い合わせ内容を入力してください">{{ old('content') }}</textarea>
        @error('content')
            <div class="error">{{ $message }}</div>
        @enderror
